...
public class AuthProvider : IAuthProvider
{
    ...
    public Users? GetUserForLogin(string Mail)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error occurred while retrieving user for login.");
        ...
        string role)
    {
        ...
        try
        {
            using (var connection = _databaseConnection.CreateConnection())
            {
                ...
                if (reader.Read())
                {
                    string status = reader["Result"].ToString() ?? string.Empty;

                    if (status == "MAIL_EXISTS")
                    {
                        result.Error = "MAIL_EXISTS";
                        return result;
                    }
